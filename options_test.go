...
func TestGroupParams(t *testing.T) {
	s := NewServer()
	group := Group(s.RouterGroup(), "/api").Param(HeaderParamType, "X-Test-Header", "test-value", WithExample("example"), WithRequiredParam())

	Get(s.RouterGroup(), "/", controller)
	Get(group, "/test", controller)
	Get(group, "/test2", controller)

	require.Equal(t, group.params, []OpenAPIParam{
		{
	...
	require.Equal(t, group.params, []OpenAPIParam{
		{
			...
			Name:        "X-Test-Header",
			Description: "test-value",
			opts:        []func(*OpenAPIParamOption){WithExample("")},
		},
	})
...
func TestGroupParamsInChildGroup(t *testing.T) {
	s := NewServer()
	group := Group(s.RouterGroup(), "/api").Param(HeaderParamType, "X-Test-Header", "test-value")

	subGroup := Group(group, "/users")

	expectedParams := []OpenAPIParam{
		{
			Type:        HeaderParamType,
			Name:        "X-Test-Header",
			Description: "test-value",
...
func TestGroupParamsNotInParentGroup(t *testing.T) {
	...
	group := Group(parentGroup, "/users").Param(HeaderParamType, "X-Test-Header", "test-value")
...
func TestGroupParamsInMainServerInstance(t *testing.T) {
	s := NewServer().RouterGroup().Param(HeaderParamType, "X-Test-Header", "test-value")
...
func TestHideGroupAfterGroupParam(t *testing.T) {
	s := NewServer()
	group := Group(s.RouterGroup(), "/api").Param(HeaderParamType, "X-Test-Header", "test-value").Hide()
